...
FUNCTIONS = {
    ...
    '15': Function(f=lambda x, y: x**2 + 6 * y**2 + x * y + x, x0=Vector(1.5, 1.1), description='f = x² + 6y² + xy + x; x₀ = (1.5, 1.1)'),
    '25_2.1': Function(f=lambda x, y: 2 * x**2 + 5 * y**2 + x * y + x, constraint=lambda x, y: x + .5 * y - 5, description='f = 2x² + 5y² + xy + x; x + 0.5y = 5'),
    '25_2.2': Function(f=lambda x, y: x**2 + 8 * y**2 + x * y + x, constraint=lambda x, y: 3 * x + y - 2, description='f = x² + 8y² + xy + x; 3x + y = 2')
}
